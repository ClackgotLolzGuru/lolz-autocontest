...
class User:
    ...
    def checkBalance(self) -> bool:
        lolzGuruResp = self.makerequest("GET",
                                           settings.lolzUrl,
                                           timeout=12.05,
                                           retries=3,
                                           checkforjs=True)
        if lolzGuruResp is None:
            return False        
        lolzGuruSoup = BeautifulSoup(lolzGuruResp.text, "html.parser")
        
        balances = lolzGuruSoup.select('a[href$=payments] span.balanceValue')
